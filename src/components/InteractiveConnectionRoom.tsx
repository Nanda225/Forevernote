...
import { 
  ...
  Layers, Smile, MessageCircle, HelpCircle, Trophy, Feather, ShieldAlert
} from "lucide-react";
...
const WAX_COLORS = [
  { name: "Imperial Red", hex: "#dc2626", glow: "shadow-red-500/50", label: "🌹 Romantic Vibe" },
  { name: "Royal Lavender", hex: "#7c3aed", glow: "shadow-purple-500/50", label: "🪻 Cozy Vibe" },
  { name: "Celestial Gold", hex: "#d97706", glow: "shadow-amber-500/50", label: "✨ Celestial Gold" },
  { name: "Midnight Teal", hex: "#0d9488", glow: "shadow-teal-500/50", label: "🌌 Secret Vibe" }
];
...
export default function InteractiveConnectionRoom({ user, profile, milestones }: InteractiveConnectionRoomProps) {
  ...
  // Commit the sealed moment permanently into the Scrapbook Milesone array
  const handleExecuteSealMilestone = async () => {
    ...
    const formattedTitle = `Sealed Memory: ${activePrompt.category} Prompt Choice 💝`;

    // Construct the joint description containing both of their sealed responses
    const jointDescription = `We completed an Interactive Connection Quest together! 
...
🤝 This moment of intimate connection was forged, sealed with a beautiful ${selectedWax.name} wax seal, and locked forever in our shared timeline.`;
    ...
    const payload = {
      ...
      aiLetterStyle: "Romantic",
      generatedLetter: `A custom sealed covenant generated during our live conversation. 
Wax Color: ${selectedWax.name}.
Forged on: ${new Date().toLocaleDateString("en-US", { month: "long", day: "numeric", year: "numeric", hour: "2-digit", minute: "2-digit" })} UTC.`,
      createdAt: serverTimestamp()
    };
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
      ...
      <div className="lg:col-span-4 bg-white p-6 rounded-3xl border border-rose-100 shadow-xs space-y-4">
        <div>
          <span className="inline-flex items-center gap-1.5 px-3 py-1 text-[9px] font-black uppercase text-pink-600 bg-pink-50 rounded-full select-none">
            <Layers className="w-3.5 h-3.5 animate-pulse" /> Intimacy Catalyst
          </span>
          <h3 className="font-display font-black text-lg text-slate-900 mt-2">Quest Prompts</h3>
              ...
              >
                <div className="flex justify-between items-center w-full">
                  <span className={`text-[9px] font-extrabold uppercase px-2 py-0.5 rounded-md ${
                    isCurrentActive ? 'bg-pink-600 font-black text-pink-100' : 'bg-slate-200 text-slate-500'
                  }`}>
                    {p.category}
                  </span>
                  {isCurrentActive && (
                ...
                <div className="absolute left-0 bottom-0 top-0 w-1 bg-pink-500 transform transition-transform origin-left scale-y-0 group-hover:scale-y-100" />
              </button>
            );
          })}
        </div>
      ...
      <div className="lg:col-span-8 space-y-6">
        ...
        {activePrompt ? (
          <div className="bg-white rounded-3xl border border-rose-100 shadow-sm p-6 space-y-6 relative overflow-hidden">
            ...
            <div className="flex justify-between items-start border-b border-rose-100 pb-4">
              <div className="space-y-1 flex-1">
                <span className="text-[9px] text-pink-600 font-extrabold tracking-widest uppercase">
                  Quest Active • Real-time Collaboration Link 🔒
                </span>
              ...
              <button
                ...
                className="px-3 py-1.5 bg-slate-100 hover:bg-rose-50 text-slate-500 hover:text-rose-600 font-bold font-mono text-[9px] rounded-lg transition-colors cursor-pointer shrink-0 uppercase tracking-wider"
              >
                Clear Quest
              </button>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                {isMyAnswerLocked ? (
                  <div className="bg-slate-50 border border-slate-200 rounded-2xl p-4 min-h-[160px] flex flex-col justify-between relative shadow-inner">
                    ...
                    <div className="flex justify-end pt-3">
                      <button
                        ...
                        className="text-[9px] text-pink-500 hover:text-pink-600 font-bold flex items-center gap-1 transition-colors cursor-pointer bg-white border border-pink-100 px-2.5 py-1 rounded-lg"
                      >
                  ...
                  <div className="space-y-3">
                    <textarea
                      ...
                      className="w-full bg-linear-to-b from-stone-50 to-stone-50/50 border border-slate-200 text-xs rounded-2xl p-4 min-h-[160px] focus:outline-hidden focus:bg-white focus:border-pink-300 resize-none leading-relaxed italic placeholder-slate-400"
                      placeholder="Write your secret heartfelt thoughts here... Your partner won't be able to open this container until both of you click Lock In!"
                      value={myAnswer}
              ...
              <div className="space-y-3">
                <div className="flex justify-between items-center">
                  ...
                  <span className={`text-[9px] font-black uppercase px-2 py-0.5 rounded-full ${
                    partnerStatus?.locked ? 'bg-emerald-100 text-emerald-700' : 'bg-rose-100 text-rose-700'
                  }`}>
                ...
                {partnerStatus?.locked ? (
                  <div className="relative overflow-hidden rounded-2xl border border-dashed border-pink-200 bg-pink-50/20 p-4 min-h-[160px] flex flex-col justify-between items-center text-center shadow-inner">
                    ...
                    {isMyAnswerLocked ? (
                      ...
                      <div className="m-auto space-y-2">
                        <Lock className="w-8 h-8 text-pink-500 animate-bounce mx-auto" />
            ...
            <AnimatePresence>
              {isMyAnswerLocked && partnerStatus?.locked && (
                ...
                >
                  <div className="absolute inset-0 bg-cover bg-center opacity-10 mix-blend-color-burn pointer-events-none" style={{ backgroundImage: `url(${new URL("../assets/images/couple_train_sunset_1780158700135.png", import.meta.url).href})` }} />

                  {/* Ritual Title */}
                  <div className="flex justify-between items-center relative z-10">
                    <div className="space-y-1">
                      <span className="flex items-center gap-1 text-[9px] uppercase font-black text-pink-400 tracking-widest">
                      ...
                      {WAX_COLORS.map((wax) => (
                        <button
                          ...
                          className={`w-6 h-6 rounded-full cursor-pointer transition border border-white/20 select-none ${
                            selectedWax.name === wax.name ? 'scale-115 ring-2 ring-pink-500 shadow-md' : 'opacity-70 hover:opacity-100 hover:scale-105'
                          }`}
                  ...
                  <div className="bg-slate-950 rounded-xl relative p-6 h-48 border border-white/5 overflow-hidden flex flex-col items-center justify-center text-center">
                      ...
                      <div className="space-y-3 z-10 m-auto">
                        ...
                        <div className="relative w-8 h-8 mx-auto flex items-center justify-center">
                          <motion.div 
                            ...
                            style={{ backgroundColor: selectedWax.hex, filter: "blur(2px)" }}
                          />
                        </div>
                        <span className="font-display font-extrabold text-[11px] uppercase tracking-widest text-pink-300 animate-pulse block">
                          Melting and Pouring Wax...
                        </span>
                        <p className="text-[8px] text-slate-400">Heating {selectedWax.name} pellets to liquid form</p>
                      </div>
                    )}
                      ...
                      <div className="space-y-3 z-10 m-auto">
                        ...
                        >
                          ...
                          <div className="w-10 h-10 rounded-full border-2 border-amber-400 bg-linear-to-b from-amber-200 to-amber-500 flex items-center justify-center mx-auto shadow-lg shadow-amber-300/30">
                            <span className="text-stone-900 font-extrabold text-xs">❤️</span>
                          </div>
                        </motion.div>
                        ...
                        <p className="text-[8px] text-slate-400">Forging everlasting bond into the scrapbook</p>
                      </div>
                    )}
                    ...
                    {sealStage === "sealed" && (
                      <div className="space-y-2 z-10 m-auto">
                        <motion.div 
                          ...
                          style={{ backgroundColor: selectedWax.hex, boxShadow: `0 0 20px ${selectedWax.hex}80` }}
                        >
                          <span className="text-white font-black text-xl select-none">❤️</span>
                          <motion.span 
                            animate={{ rotate: 360 }} 
                    ...
                    <div 
                      ...
                      style={{ background: `radial-gradient(circle, ${selectedWax.hex} 0%, transparent 70%)` }}
                    />
                  </div>
          ...
          <div className="bg-white rounded-3xl border border-rose-100 shadow-xs p-10 text-center space-y-4">
            <span className="text-4xl block leading-none">💬</span>
            <div className="space-y-2 max-w-sm mx-auto">
              <h4 className="font-display font-black text-lg text-slate-800">
                Interactive Connection Forum
              </h4>
            ...
            <div className="pt-2">
              <span className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-pink-50 text-pink-600 rounded-xl text-[10.5px] font-extrabold uppercase border border-pink-100">
                👉 Select a relationship prompt on the left to activate
              </span>
            </div>
          </div>
        )}
        ...
        <div className="bg-linear-to-r from-purple-500 to-pink-550 rounded-3xl p-6 text-white relative overflow-hidden shadow-xs">
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="space-y-1">
              <span className="text-[9.5px] font-black uppercase text-pink-100 tracking-wider">How Sealing Works</span>
              <h4 className="font-display font-black text-sm text-white">Digital Co-Covenant</h4>
              <p className="text-[10.5px] text-pink-50/80 leading-normal max-w-2xl">
                True relationship scrapbook artifacts are constructed together. When a prompt is loaded, keep client dashboards active. Both type private answers and lock them to keep the answers a romantic surprise. Once both lock, hit <strong>Forge &amp; Seal</strong>. The app executes an interactive wax pouring seal simulation, publishes the combined story layout into the scrapbook feed, and logs it in the vault permanent archive.
              </p>
            </div>
          </div>
        </div>
